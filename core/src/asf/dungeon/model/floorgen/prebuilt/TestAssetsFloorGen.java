...
public class TestAssetsFloorGen implements FloorMapGenerator, FloorMap.MonsterSpawner {
        ...
        public FloorMap generate(Dungeon dungeon, FloorType floorType, int floorIndex) {
                ...
                for (int y = 1; y <= potionValues.length; y++) {
                        dungeon.addToken(TokenFactory.loot(dungeon, new PotionItem(dungeon, potionValues[y - 1], 24)), floorMap, 1, y);
                }

                ScrollItem.Type[] scrollValues = ScrollItem.Type.values();
                for (int y = 1; y <= scrollValues.length; y++) {
                        dungeon.addToken(TokenFactory.loot(dungeon, new ScrollItem(dungeon, scrollValues[y - 1], 24)), floorMap, 3, y);
                }

                BookItem.Type[] bookValues = BookItem.Type.values();
                for (int y = 1; y <= bookValues.length; y++) {
                        dungeon.addToken(TokenFactory.loot(dungeon, new BookItem(dungeon, bookValues[y - 1])), floorMap, 5, y);
                }
                ...
                for (int x = 4; x < bookValues.length + 4; x++) {
                        ...
                        dungeon.addToken(t, floorMap, x, tileData.length - 2);
                }
                ...
                for (int y = 0; y < potionValues.length; y++) {
                        ...
                        dungeon.addToken(fountainToken, floorMap, tileData[0].length() - 2, tileData.length - y - 2);
                }
        ...
        public void spawnMonsters(Dungeon dungeon, FloorMap floorMap) {
                ...
                if (countTeam1 == 0) {
                        ...
                        //new FullAgroLogic(1)
                        Token t = TokenFactory.characterToken(dungeon, modelId, new FsmLogic(1, new Sector(7, 6, 12, 10), Monster.Sleep),floorMap );
